...
LiquidCrystal lcd(12, 11, 5, 4, 3, 2); //pini LCD
volatile int s = 0, m = 0, h = 0; // sec min ora
volatile bool one_second = false;// flag 1s
volatile bool two_second = false;// flag 2s
...
{ int min = 0;
  ...
  {
    lcd.print(" TInc:");
    moving_sp = dt_inc * timpu + T1;
    rest = timp_inc - timpu;
  }
  else if (timpu <= (timp_inc + timp_men))
  { lcd.print(" TMen:");
    rest = (timp_inc + timp_men) - timpu;
  }
  else if (timpu <= (timp_inc + timp_men + timp_rac))
  { lcd.print(" TRac:");
    if (flag2 == false)
    {
      flag2 = true;
    ...
    moving_sp = dt_rac * (timpu - (timp_inc + timp_men)) + T2;
    rest = (timp_inc + timp_men + timp_rac) - timpu;
  }
  else
    lcd.print("Oprit: ");
  min = rest / 60;
...
void loop() {
  if (one_second)
  {
    if (two_second)
    {
      temperatura = read_temp(); //citire senzor temp
      lcd.clear();
      two_second = false;
    }
